...
def read_mysql2pic(path, filename, config):
    ...
    try:
        conn = pymysql.connect(host=config['host'],
                               ...
                               use_unicode=True)
        cursor = conn.cursor()
        cursor.execute("select diffimg from diff where id = '{}'".format(filename))
...
if __name__ == '__main__':
    ...
    read_mysql2pic('D:\\学习系列\\毕业论文-定档\\0\\pyDiff\\test.png', args.rowid, my_config)
    #cd C:\Users\hy\Desktop\论文相关\0\pyDiff && python temp.py --matha sin(5*x)+7 --fname FromTemp --uid 0