...
def gen():
    ...
    plt.text(0.5,
             0.5,
             #务必保持下行在第36行
             r'$\mathrm{Diff}(x) = \sin{\left({x}\right)} + \frac{{1}}{x}$'
             ,ha='center'
             ,va='center'
             , fontsize=20)
    try:
        ...
        # sys.exit(1)
        return
def write_pic2mysql(diff,config,uid,fx,rowid):
    """
    读取图片写入数据库
    :param name: 读取的图片的名字
    ...
    try:
        conn = pymysql.connect(host=config['host'],
                               ...
                               use_unicode=True)
        cursor = conn.cursor()
        #update diff set diffImg='',diff='',fx='' where uid=0
        #sql = "update latex set latex.img=%s,latex.name='{1}' where uid='{0}'".format(uid,filename,gen())
        sql = "update diff set diffImg=%s,diff='{1}',fx='{2}',uid={0} where id='{3}'".format(uid,diff,fx,rowid)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--diff', type=str, default = 'sin(x)')
    parser.add_argument('--fx', type=str, default = 'cos(x)')
    ...
    write_pic2mysql(args.diff,my_config,args.uid,args.fx,args.rowid)
    print("执行完成")
